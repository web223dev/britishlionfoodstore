...
if ( !class_exists('pcfme_PluginUpdateChecker_2_2', false) ):
 ...
 * @access public
 */
class pcfme_PluginUpdateChecker_2_2 {
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		$this->purchase_url    = ''.site_url().'/wp-admin/admin.php?page=pcfme_plugin_options&tab=pcfme_extra_settings';
		$this->is_code_valid   =  get_option('pcfme_activation_status');
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			$pluginInfo = pcfme_PluginInfo_2_2::fromJson($result['body'], $this->debugMode);
	...
	public function requestUpdate(){
		...
		return pcfme_PluginUpdate_2_2::fromPluginInfo($pluginInfo);
	}
	...
	public function checkForUpdates(){
		pcfme_verify_envato_purchase_code();
		$installedVersion = $this->getInstalledVersion();
	...
	public function getUpdateState() {
		...
		if ( !empty($state) && isset($state->update) && is_object($state->update) ){
			$state->update = pcfme_PluginUpdate_2_2::fromObject($state->update);
		}
		return $state;
	}
		...
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
			    ...
			    if (($this->is_code_valid) == "active") {
					...
					$message= sprintf(
							'A new version of woocommerce easy checkout field editor plugin is available. Please enter valid <a href="%s"><b> purchase code</b></a> to get the update.',
							$this->purchase_url
						);
				}
...
if ( !class_exists('pcfme_PluginInfo_2_2', false) ):
...
if ( !class_exists('pcfme_PluginUpdate_2_2', false) ):
...
class pcfme_PluginUpdate_2_2 {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		$pluginInfo = pcfme_PluginInfo_2_2::fromJson($json, $triggerErrors);
		if ( $pluginInfo != null ) {
	...
	public function toWpFormat(){
		...
		$is_code_valid = get_option("pcfme_activation_status");
...
PucFactory::addVersion('PluginUpdateChecker', 'pcfme_PluginUpdateChecker_2_2', '2.2');
PucFactory::addVersion('PluginUpdate', 'pcfme_PluginUpdate_2_2', '2.2');
PucFactory::addVersion('PluginInfo', 'pcfme_PluginInfo_2_2', '2.2');
...
if ( !class_exists('PluginUpdateChecker', false) ) {
	class PluginUpdateChecker extends pcfme_PluginUpdateChecker_2_2 { }
}

if ( !class_exists('PluginUpdate', false) ) {
	class PluginUpdate extends pcfme_PluginUpdate_2_2 {}
}

if ( !class_exists('PluginInfo', false) ) {
	class PluginInfo extends pcfme_PluginInfo_2_2 {}
}
